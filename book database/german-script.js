...
const germanBooks = allBooks.filter(book => book.language === 'de');

function filterBooks() {
  const level = levelSelect.value;
  const genre = genreSelect.value;

  const filtered = germanBooks.filter(book => {
...
function renderBooks(bookList) {
  if (!bookList.length) {
    bookGrid.innerHTML = '';
    emptyState.textContent = 'No German graded readers match the selected filters. Try changing the level or genre.';
