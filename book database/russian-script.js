...
const russianBooks = books.filter(book => book.language === 'ru');

function filterBooks() {
  const level = levelSelect.value;
  const genre = genreSelect.value;

  const filtered = russianBooks.filter(book => {
...
function renderBooks(bookList) {
  if (!bookList.length) {
    bookGrid.innerHTML = '';
    emptyState.textContent = 'No Russian graded readers match the selected filters. Try changing the level or genre.';
