...
const spanishBooks = allBooks.filter(book => book.language === 'es');

function filterBooks() {
  const level = levelSelect.value;
  const genre = genreSelect.value;

  const filtered = spanishBooks.filter(book => {
...
function renderBooks(bookList) {
  if (!bookList.length) {
    bookGrid.innerHTML = '';
    emptyState.textContent = 'No Spanish graded readers match the selected filters. Try changing the level or genre.';
