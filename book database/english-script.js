...
const englishBooks = allBooks.filter(book => book.language === 'en');

function filterBooks() {
  const level = levelSelect.value;
  const genre = genreSelect.value;

  const filtered = englishBooks.filter(book => {
...
function renderBooks(bookList) {
  if (!bookList.length) {
    bookGrid.innerHTML = '';
    emptyState.textContent = 'No English graded readers match the selected filters. Try changing the level or genre.';
