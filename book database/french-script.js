...
const frenchBooks = allBooks.filter(book => book.language === 'fr');

function filterBooks() {
  const level = levelSelect.value;
  const genre = genreSelect.value;

  const filtered = frenchBooks.filter(book => {
...
function renderBooks(bookList) {
  if (!bookList.length) {
    bookGrid.innerHTML = '';
    emptyState.textContent = 'No French graded readers match the selected filters. Try changing the level or genre.';
