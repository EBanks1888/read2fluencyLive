...
const hindiBooks = books.filter(book => book.language === 'hi');

function filterBooks() {
  const level = levelSelect.value;
  const genre = genreSelect.value;

  const filtered = hindiBooks.filter(book => {
...
function renderBooks(bookList) {
  if (!bookList.length) {
    bookGrid.innerHTML = '';
    emptyState.textContent = 'No Hindi graded readers match the selected filters. Try changing the level or genre.';
