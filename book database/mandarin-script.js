...
const mandarinBooks = books.filter(book => book.language === 'zh');

function filterBooks() {
  const level = levelSelect.value;
  const genre = genreSelect.value;

  const filtered = mandarinBooks.filter(book => {
...
function renderBooks(bookList) {
  if (!bookList.length) {
    bookGrid.innerHTML = '';
    emptyState.textContent = 'No Mandarin graded readers match the selected filters. Try changing the level or genre.';
